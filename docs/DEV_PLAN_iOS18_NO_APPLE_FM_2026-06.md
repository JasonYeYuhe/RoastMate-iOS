...
- **⚠️ 计划遗漏的第二处 iOS-26 触面 = 语音**:`VoiceVentTranscriber`(iOS-26 `SpeechAnalyzer`/`DictationTranscriber`/`AssetInventory`)同样硬依赖 26,降目标即编译不过。**根因同 §1.2(全仓 0 处运行期门 → 降目标暴露所有 26-only API,不止 FM)。** 处置:整类 `@available(iOS 26, *)`,`VoiceVentSheet` 同标,`RoastGeneratorView` 的 gate 查询 + sheet 包 `if #available(iOS 26, *)` —— **iOS 18 上语音入口直接隐藏(voiceGate 留 nil),用户改用打字**。本期不为 iOS 18 重写 SFSpeechRecognizer 路径(超出「无新 API」范围)。
