...
struct ShareCardView: View {
    ...
    private var comeback: some View {
        VStack(alignment: .leading, spacing: 20 * s) {
            ...
            // Children MUST be written out statically: ViewThatFits picks
            // among the ViewBuilder's own children, and a ForEach would
            // collapse into a single candidate, defeating the whole point.
            ViewThatFits(in: .vertical) {
                comebackText(size: 66)
                comebackText(size: 54)
                comebackText(size: 44)
                comebackText(size: 36)
                // Final fallback: smallest step, allowed to scale further
                // rather than clip. Reaching this means the line is far longer
                // than anything the generator should produce.
                comebackText(size: 30).minimumScaleFactor(0.5)
            }
        }
    }
